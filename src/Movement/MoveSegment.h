...
// On SAMC21/RP2040 the remaining per-step cost in CalcNextStepTimeFull is the soft-float arithmetic itself:
...
// SAMC21 and RP2040, so this is ~16 clocks inline; the plain C expression (uint64_t)a * b would be a call to
...
static inline int64_t MulStepByCoeff(int32_t n, int64_t coeff) noexcept
{
	uint32_t un;
	bool neg;
...
// qfp_fsqrt, which itself keeps 24 significant bits, so truncation instead of round-to-nearest loses at most
// 2^-24 relative - no worse than the float path this replaces, which accumulated 2-3 rounded soft-float
// operations in the same value. Like the helper above, the shifting is composed from 32-bit operations.
